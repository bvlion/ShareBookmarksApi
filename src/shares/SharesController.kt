...
class SharesController {
  ...
  fun getShares(userId: Int) = SharesModel.GetList(
      SharesDao.join(UsersDao, JoinType.INNER, additionalConstraint = { SharesDao.shareUserId eq UsersDao.id })
        .select(SharesDao.id, SharesDao.itemsId, UsersDao.email, SharesDao.ownerType, SharesDao.updated)
        .where { (SharesDao.ownerUserId eq userId) and SharesDao.deleted.isNull() }
